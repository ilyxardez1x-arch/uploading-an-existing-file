...
# ========== НАСТРОЙКИ ==========
TELEGRAM_TOKEN = "8208320562:AAEil8ttxNEGcjfTLcwry9CqRIeGnjM8Ft0"
GROQ_API_KEY   = "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"
OWNER_ID       = 6210516253   # ← твой ID, главный админ навсегда
# ================================
...
def kb_admin(is_owner: bool):
    ...
    if is_owner:
        ...
        rows.append([
            InlineKeyboardButton(text="📋 Список админов", callback_data="adm_listadmins"),
        ])
    rows.append([
...
async def cb_adm_setadmin(cb: CallbackQuery):
    if cb.from_user.id != OWNER_ID:
        await cb.answer("❌ Только владелец может это делать", show_alert=True)
        return
...
async def handle_photo(msg: Message):
    ...
    if not ok:
        ...
        await msg.answer(
            f"❌ <b>Лимит фото исчерпан</b> ({used}/{limit} сегодня)",
...
async def handle_text(msg: Message):
    ...
    if uid in waiting_setadmin and uid == OWNER_ID:
        ...
        if msg.text.strip().isdigit():
            target = int(msg.text.strip())
            if target == OWNER_ID:
                await msg.answer("👑 Это уже ты — главный владелец!", reply_markup=kb_back_admin())
                return
            await get_user(target)
    ...
    if uid in waiting_remadmin and uid == OWNER_ID:
        ...
        if msg.text.strip().isdigit():
            ...
            await msg.answer(f"✅ У пользователя <code>{target}</code> снята админка", reply_markup=kb_back_admin(), parse_mode="HTML")
        else:
            await msg.answer("❌ Неверный ID", reply_markup=kb_back_admin())
        return
    ...
    if not ok:
        ...
        await msg.answer(
            f"❌ <b>Лимит запросов исчерпан</b> ({used}/{limit} сегодня)\n\nЛимиты обновятся завтра.",
            reply_markup=kb, parse_mode="HTML"
        )
        return
